...
logit_func_path=RN50_evol/${srcdata}_inst${inst}_nt${ntest_per_class}/${method}_logit/ael_popsize${popsize}_npop${npop}_sc${sample_class}/results/pops_best/population_generation_${npop}.json
save_prefix=fsl_${srcdata}_inst${inst}_nt${ntest_per_class}_${method}_logit_ael_popsize${popsize}_npop${npop}_sc${sample_class}

if [ ${method} = "ape" ]
  then
    feat_func_path=seeds/feat_selection.json
  else
    feat_func_path="none"
fi
echo ${feat_func_path}
...
do
  ...
  do
     ...
     save_path=${save_dir}/${data}/${save_prefix}_shots${shots}_seed${seed}_optuna${optuna}.txt
     if [ -f "$save_path" ]; then
