...
exp_output_path = f"{net}_evol/{srcdata0}_inst{inst}_nt{ntest_per_class}/{method}_logit/{evomethod}_popsize{ec_pop_size}_npop{ec_n_pop}_sc{sample_class}"
makedirs(exp_output_path)

if method == "ape":
    feat_sel_path = "seeds/feat_selection.json"
    exp_seed_path = f"seeds/logit_func_opt-ape.json"
elif method == "gda":
    feat_sel_path = ""
    exp_seed_path = f"seeds/logit_func_opt-gda-wo-fs.json"
elif method == "tip":
    feat_sel_path = ""
    exp_seed_path = f"seeds/logit_func_opt-tip-wo-fs.json"
else:
    raise

# *************** optimization for logit computation ***************

set_logger(f"{exp_output_path}/log.txt")
...
paras.set_paras(method = evomethod,    # ['ael','eoh']
                ...
                exp_use_seed = True,
                exp_seed_path = exp_seed_path,
                exp_output_path = exp_output_path,
                log_file = f"{exp_output_path}/log.txt",
            )
save_params(paras)
# initilization
