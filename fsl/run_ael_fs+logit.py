...
exp_n_proc = 0
ntest_per_class = int(sys.argv[6])
fs_exp_output_path = f"{net}_evol/{srcdata0}_inst{inst}_nt{ntest_per_class}/{method}_fs+logit-s1/{evomethod}_popsize{ec_pop_size}_npop{ec_n_pop}_sc{sample_class}"
logit_exp_output_path = f"{net}_evol/{srcdata0}_inst{inst}_nt{ntest_per_class}/{method}_fs+logit-s2/{evomethod}_popsize{ec_pop_size}_npop{ec_n_pop}_sc{sample_class}"
...
problem_params = {
    ...
    "shots": [1,2,4,8,16],
    "inst_per_shots": inst,
    "logit_func_path": f"seeds/logit_func_opt-ape.json",
...
paras.set_paras(method = evomethod,    # ['ael','eoh']
                ...
                log_file = f"{fs_exp_output_path}/log.txt",
            )
save_params(paras)
# initilization
...
problem_params = {
    ...
    "inst_per_shots": inst,
    "feat_sel_path": f"{fs_exp_output_path}/results/pops_best/population_generation_{ec_n_pop}.json",
    "init_alpha": [0.1, 1.0, 10.0],
    "init_w0": [0.5], 
    "init_percent": [0.7],
    "sample_class": sample_class,
