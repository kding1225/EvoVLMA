...
class MyDataset(torchvision.datasets.ImageFolder):
    ...
    def __init__(self, root_dir, split, transform):
        ...
        self.class_to_idx = {c:i for i, c in enumerate(imagenet_classes)}
...
class ImageNet():
    ...
    def __init__(self, root, num_shots, preprocess):
        ...
        self.train = MyDataset(self.image_dir, split='train', transform=train_preprocess)
        self.val = MyDataset(self.image_dir, split='val', transform=test_preprocess)
        self.test = MyDataset(self.image_dir, split='val', transform=test_preprocess)
...
class ImageNetV2(torchvision.datasets.ImageFolder):
    ...
    def __init__(self, root_dir, transform):
        image_dir = os.path.join(root_dir, "imagenetv2-matched-frequency-format-val")
        super().__init__(image_dir, transform)

        old_cid_to_new_cid = {}
        for k, v in self.class_to_idx.items():
            old_cid_to_new_cid[v] = int(k)
        
        self.samples = [(s[0], old_cid_to_new_cid[s[1]]) for s in self.samples]
        self.targets = [old_cid_to_new_cid[t] for t in self.targets]
        
        self.classes = imagenet_classes
        self.class_to_idx = {c:i for i, c in enumerate(imagenet_classes)}


class ImageNetSketch(torchvision.datasets.ImageFolder):
