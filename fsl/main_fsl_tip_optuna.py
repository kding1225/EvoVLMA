...
def objective(trial, cfg, cache_keys, cache_values, val_features, val_labels, clip_weights):

    alpha = trial.suggest_float("alpha", 1e-9, cfg["ub"], log=True)
    beta = trial.suggest_float("beta", 1e-9, cfg["ub"], log=True)

    R_fF = val_features @ cache_keys.t()
...
def TIP(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights):
    ...
    R_fW = 100. * test_features @ clip_weights
    R_fF = test_features @ cache_keys.t()
    cache_logits = ((-1) * (params["beta"] - params["beta"] * R_fF)).exp() @ cache_values
    logits = R_fW + cache_logits * params["alpha"]
    time2 = time.time()

    print("used time: "+str(time2-time1)+"s, test shape: ", test_features.shape)
    
    acc = cls_acc(logits, test_labels)
    print("**** TIP's test accuracy: {:.2f}. ****\n".format(acc))
...
def main():
    ...
    if args.dtype == "half":
        TIP(cfg, cache_keys.half(), cache_values.half(), val_features.half(), val_labels, test_features.half(), test_labels, clip_weights.half())
    else:
        TIP(cfg, cache_keys.float(), cache_values.float(), val_features.float(), val_labels, test_features.float(), test_labels, clip_weights.float())
