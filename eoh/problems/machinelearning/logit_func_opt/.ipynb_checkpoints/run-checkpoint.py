...
class LogitFuncOpt():
    def __init__(self, problem_params) -> None:
        ...
        self.prompts = GetPrompts(problem_params["feat_sel_path"]!="")
        ...
        self.feat_sel_path = problem_params["feat_sel_path"]
        ...
        self.init_w0 = problem_params['init_w0']
        self.init_percent = problem_params['init_percent']
    ...
    def call_feat_selection(self, data_path, w0, w1, topk):

        with open(self.feat_sel_path, "r") as f:
    ...
    def call_eval(self, code, indices, data_path, max_retries=1):
    ...
    def compute_error(self, code):
        ...
        def run_inst(inst_path, init_w0, init_percent):
            if self.feat_sel_path == "":
                indices = None
            else:
                indices = self.call_feat_selection(inst_path, init_w0, 1-init_w0, int(init_percent*self.feat_dim))

            acc, pred = self.call_eval(code, indices, inst_path)
            return acc, pred

        accs = []
        preds = []
        num = 0
        for init_w0 in self.init_w0:
            for init_percent in self.init_percent:

                acc, pred = run_inst(self.instance_paths[0], init_w0, init_percent)       
                if acc is None:
                    return None, None

                results = [None for _ in self.instance_paths]
                with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                    future_to_index = {executor.submit(run_inst, self.instance_paths[i], init_w0, init_percent): i for i in range(len(self.instance_paths))}
